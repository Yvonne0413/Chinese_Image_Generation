...
# -*- coding: utf-8 -*-
# @Time : 5/20/2020 8:07 AM
# @Author : Zhuoyi Huang
# @File : get_rough_image.py
...
from utils.loss_map import compute_loss_map
import utils.sqlite_database as sqlite_database
from utils.sqlite_database import open_database, close_database

"""
Converting numpy array to binary and back for insertion in sql
"""

FILL_DATABASE = False  # if True, transform database.csv to database.db.Keep False for now.
DATABASE_PATH = "../database.db"
UPDATE_DATABASE = False  # if True, update database. Keep False for now.
TEXT_DATABASE_PATH = "../inputFiles/database4.0.txt"
COMPUTE_AND_IMPORT_ARRAYS = True  # process the style.input multiple styles
CHARACTER_IMAGES_PATH = "../../data/interFiles/FontImages/"
PROCESS_STYLES = False
PATH_TO_PROCESS = "../characterImages/Songti-for-training/"
OUTPUT_PATH = "../../data/interFiles/FontImages/"
SHOW_ARRAYS = False
SHELL = False
IMAGE_WIDTH = 1000
IMAGE_HEIGHT = 1000
...
def computeAndImportArrays(style):
    ...
    def getBaseImages(style):
        ...
        roughImageDic = {}
        arrayDic = {}
        for file in os.listdir(CHARACTER_IMAGES_PATH + style + "/all_1000/"):
            if file.endswith(".png"):
                uid = file[:-4]
                charDef = charDefDic.get(uid)
                if charDef != None:
                    try:
                        image = Image.open(CHARACTER_IMAGES_PATH + style + "/all_1000/" + file)
    ...
    def createCharImage(uid):
        try:
            ...
            print("Error encountered for unicode: ", cDef.uid)
            print("Error message: ", error)
        if lid == 0:
            raise Exception("Base image for character id " + uid + " was not provided")
        else:  # only process lid==1 zuoyou
            compImgs = []
            for i in compIds:
    ...
    def addLossMapArrays():
        ...
        for uid, ac in arrayDic.items():
            ...
            ac.add_loss_map(lossMap)
        print("")

    print("Importing arrays for style: ", style)
    charDefDic = sqlite_database.get_char_def_dic()
    ...
    for uid, ac in arrayDic.items():
        if ac.is_complete():
            ...
            # if want to make all the
            # images[1].save(OUTPUT_PATH + "/rough_1000/" + ac.style + "_" + ac.uid + ".png") # different folder
...
def sqlShell():  # command line to look in to the database.db
    (conn, cur) = open_database()
    buffer = ""
...
def main():
    ...
    if COMPUTE_AND_IMPORT_ARRAYS:
        ...
        for style in styles:
            rough_directory = os.path.dirname('../../data/interFiles/FontImages/{}/rough_1000/'.format(style))
            origin_directory = os.path.dirname('../../data/interFiles/FontImages/{}/origin_1000/'.format(style))
            if not os.path.exists(rough_directory):
                os.makedirs(rough_directory)
            if not os.path.exists(origin_directory):
                os.makedirs(origin_directory)
